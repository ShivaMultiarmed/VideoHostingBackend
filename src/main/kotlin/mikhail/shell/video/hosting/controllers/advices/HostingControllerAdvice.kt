...
//    }
    ...
    fun handleNotValidArguments(e: MethodArgumentNotValidException): ResponseEntity<Map<String, String>> {
        return createErrorResponse(e.bindingResult)
    }
}

fun createErrorResponse(bindingResult: BindingResult): ResponseEntity<Map<String, String>> =
    ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
        mutableMapOf<String, String>().apply {
            bindingResult.allErrors.forEach {
                this@apply[(it as FieldError).field] = it.defaultMessage ?: UnexpectedError.toString()
            }
        }
    )